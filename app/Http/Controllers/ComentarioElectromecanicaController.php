...
use App\Models\ComentarioElectromecanica;
use App\Http\Resources\Resource;
use Illuminate\Support\Facades\Validator;

class ComentarioElectromecanicaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        }$comentarios = new ComentarioElectromecanica();
        ...
        $comentarios->save();
        return response()->json(["carreras" => $comentarios, "message"=>"El comentario se ha creado satisfactoriamente"], 200);

    }
    //FUNCION PARA VER LOS COMENTARIOS
    public function index (Request $request){

        $comentarios = ComentarioElectromecanica::all();
...
//FUNCION PARA VER UN COMENTARIOS
    public function show (Request $request, $id){
        ...
        $comentarios = ComentarioElectromecanica::find($id);
        if($comentarios){
            return response()->json([
                'mesagge' => 'Semestres a vizualizarse',
                'data'=> $comentarios
...
//FUNCION PARA ACTUALIZAR COMENTARIOS
    public function update (Request $request, $id){
        ...
        if($comentarios){
            ...
            return response()->json([
                'message'=> 'No existe ninguna semestre con ese id.'
